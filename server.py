...
def location_query():
    ...
    sql = "SELECT Name,Location FROM RETAIL.LIQUOR WHERE Age ='{}' AND Type='{}' AND Price>{} AND Price<{}".format(params["age"],params["type"],int(params["min_price"]),int(params["max_price"]))
    print(sql)
    #sql = "SELECT Location FROM RETAIL.LIQUOR WHERE Type = '%s' AND Price<='%d'"
    cur.execute( sql )
    #cur.execute( sql % params['type'], int(params['price']) )
    #print(sql)
    # cur.execute( sql )
    print(type(params['type']))
    print(type(params['age']))
    print((int(params['min_price'])))
    print((int(params['max_price'])))
